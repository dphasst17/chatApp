...
const SocialLogin = () => {
  ...
  const googleLog = useGoogleLogin({
    onSuccess: (tokenResponse) =>
      fetch(
        ...
        .then((data) =>
          handleSocialLogin(data.email, data.given_name, data.picture),
        ),
  });
  const handleSocialLogin = (e: string, n: string, p: string) => {
    socialLogin(e, n, p).then((res) => {
      if (res.status === 200) {
  ...
  return (
    <div className="w-full h-full flex flex-wrap justify-center items-center content-center gap-8 font-[sans-serif]">
      <div className="w-full h-[60px] flex items-center justify-center text-black text-5xl font-bold">
        Sign In Chat App
      </div>
      <FacebookLogin
        ...
        onProfileSuccess={(response) =>
          handleSocialLogin(
            ...
            response.picture ? (response.picture.data.url as string) : "",
          )
        }
